...
{
    public partial class FormFeedforward : Form
    {
        FeeditemDAL feeditemDal;
        FeedforwardDAL feedforwardDAL;
        AssignmentDAL assignmentDal;
        UserDAL userDAL;
        SupervisorDAL supervisorDAL;
        StudentDAL studentDAL;
        
        Feedforward selectedItem;

        public FormFeedforward(DALs dals)
        {
            InitializeComponent();

            // Contains all of the dals inside
            this.feedforwardDAL = dals.feedforwardDAL;
        ...
        private void PopulateDataGridView()
        {
            ...
            PropertyDescriptorCollection props = TypeDescriptor.GetProperties(typeof(Feedforward));
            DataTable dt = new DataTable();

            foreach (PropertyDescriptor p in props)
                dt.Columns.Add(p.Name, p.PropertyType);

            for (int i = 0; i < feedforwardDAL.Feedforwards.Count; i++)
            {
                var item = feedforwardDAL.Feedforwards[i];
        ...
        private void PopulateInputs(Feedforward item)
        {
        ...
        {
            if (selectedItem != null)
                feedforwardDAL.Delete(selectedItem);
            ClearInputs();
            RefreshAndPopulate();
        }
        ...
        private void SelectRowFromDataGridView(int Index)
        {
            ...
            if (Index != -1)
            {
                ...
                selectedItem = new Feedforward(
                    (int)dgvFeedback.Rows[Index].Cells[1].Value,
                    (DateTime)dgvFeedback.Rows[Index].Cells[2].Value,
                    (bool)dgvFeedback.Rows[Index].Cells[3].Value,
                    (int)dgvFeedback.Rows[Index].Cells[4].Value,
                    (int)dgvFeedback.Rows[Index].Cells[5].Value,
                    (int)dgvFeedback.Rows[Index].Cells[6].Value,
                    (int)dgvFeedback.Rows[Index].Cells[7].Value,
                    (string)dgvFeedback.Rows[Index].Cells[8].Value,
